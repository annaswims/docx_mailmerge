...
describe "integration test", integration: true do
  ...
  let(:complex_input_file) { "#{base_path}/complex/unmerged/doc.docx" }
  ...
  context "should process in incoming docx" do
    it "generates a valid zip file (.docx)" do
      output_file_path = "#{output_dir}/IntegrationTestSimple.docx"
      DocxMailmerge::DocxCreator.new(simple_input_file).generate_docx_file(data, output_file_path)
     ...
     it "generates a valid zip file (.docx)" do
      output_file_path = "#{output_dir}/IntegrationTestComplex.docx"
      DocxMailmerge::DocxCreator.new(complex_input_file).generate_docx_file(data, output_file_path )
    ...
    it "returns an array of merge fields" do
      merge_fields = DocxMailmerge::DocxCreator.new(complex_input_file).merge_field_names
      merge_fields.should =~  %w{First_Name Last_Name}
    end
